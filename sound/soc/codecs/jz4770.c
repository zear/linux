...
static unsigned int jz_icdc_read_reg(struct snd_soc_component *codec,
				     unsigned int reg)
{
	struct jz_icdc *jz_icdc = snd_soc_component_get_drvdata(codec);
	unsigned int val;
...
static void jz_icdc_update_reg(struct snd_soc_component *codec, unsigned int reg,
...
static int jz_icdc_set_bias_level(struct snd_soc_component *codec,
...
static int jz_icdc_digital_mute(struct snd_soc_dai *dai, int mute)
{
	...
	change = snd_soc_component_update_bits(codec, JZ_ICDC_CR_DAC, 1 << 7,
					       mute << 7);
...
static int jz_icdc_suspend(struct snd_soc_component *codec)
{
	return snd_soc_component_force_bias_level(codec, SND_SOC_BIAS_OFF);
}

static int jz_icdc_resume(struct snd_soc_component *codec)
{
	return snd_soc_component_force_bias_level(codec, SND_SOC_BIAS_STANDBY);
}
...
static void jz_icdc_codec_init_regs(struct snd_soc_component *codec)
{
	struct jz_icdc *jz_icdc = snd_soc_component_get_drvdata(codec);
...
static int jz_icdc_codec_probe(struct snd_soc_component *codec)
{
	struct snd_soc_dapm_context *dapm = snd_soc_component_get_dapm(codec);
	struct jz_icdc *jz_icdc = snd_soc_component_get_drvdata(codec);
	...
	if (ret)
		return ret;
	ret = snd_soc_add_component_controls(codec, mic_cfg->controls,
...
static void jz_icdc_codec_remove(struct snd_soc_component *codec)
{
	struct jz_icdc *jz_icdc = snd_soc_component_get_drvdata(codec);
...
static const struct snd_soc_component_driver jz_icdc_soc_codec_dev = {
...
static int jz_icdc_probe(struct platform_device *pdev)
{
	...
	ret = snd_soc_register_component(&pdev->dev, &jz_icdc_soc_codec_dev,
					 &jz_icdc_dai, 1);
	if (ret) {
