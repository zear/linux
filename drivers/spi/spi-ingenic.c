				       ...
				       struct spi_message *message)
{
	...
	unsigned int ssicr0 = REG_SSICR0_EACLRUN, ssicr1 = 0;
...
static int spi_ingenic_prepare_hardware(struct spi_controller *ctlr)
{
	...
	regmap_clear_bits(priv->map, REG_SSICR0, 0xffff);
	regmap_clear_bits(priv->map, REG_SSICR1, 0xffff);
	regmap_clear_bits(priv->map, REG_SSISR, 0xffff);
