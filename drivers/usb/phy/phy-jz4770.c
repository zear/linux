...
static int jz4770_phy_vbus_changed(struct notifier_block *nb,
				   unsigned long state, void *data)
{
	struct jz4770_phy *priv = container_of(nb, struct jz4770_phy,
					       phy.vbus_nb);
	struct usb_phy *phy = &priv->phy;

	dev_notice(priv->dev, "VBUS changed to state %lu", state);

	if (state) {
		usb_phy_set_charger_state(&priv->phy, USB_CHARGER_PRESENT);
		atomic_notifier_call_chain(&phy->notifier, USB_EVENT_VBUS, phy);
	} else {
		usb_phy_set_charger_state(&priv->phy, USB_CHARGER_ABSENT);
...
static int jz4770_phy_id_changed(struct notifier_block *nb,
				 unsigned long state, void *data)
{
	struct jz4770_phy *priv = container_of(nb, struct jz4770_phy,
					       phy.id_nb);
...
static int jz4770_phy_probe(struct platform_device *pdev)
{
	...
	priv->phy.vbus_nb.notifier_call = jz4770_phy_vbus_changed;
	priv->phy.id_nb.notifier_call = jz4770_phy_id_changed;
